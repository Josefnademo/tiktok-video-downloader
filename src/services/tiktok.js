...
export async function getVideoInfo(inputUrl) {
  ...
  try {
    ...
    // NOTE: This internal API allows downloading without watermarks directly from TikTok servers
    ...
    // This is the direct URL from TikTok servers (no watermark usually on this endpoint)
    const downloadUrl = videoItem.video.play_addr.url_list[0];

    // ADAPTER: We format this to match what your Front-end expects
    const qualities = [
      {
    ...
    return {
      ...
      qualities: qualities, // Important: This keeps your App.js working
    };
  } catch (error) {
